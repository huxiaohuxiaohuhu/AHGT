...
Thank you for visiting this repository. This project proposes Aspect-guided heterogeneous graph transformer (AHGT) 
...
You can generate layer-wise oversmoothing figures for a trained checkpoint with:

```bash
python analyze_oversmoothing.py \
	--dataset twitter2017 \
	--model_file ./logXMTest/twitter2017/pytorch_model.bin \
```

The script saves a PNG/PDF figure together with CSV and text summaries of the layer-wise cosine similarity and feature variance.

Note: to make the Graph Transformer comparison meaningful, use a checkpoint trained with `--addGCN 2`. The checkpoints currently stored under `logXMTest/` were trained with `--addGCN 1`, so they are only suitable for validating the pipeline, not for final Graph Transformer claims.

## Graph Transformer training

To train a real PHEB checkpoint, use the dedicated script:

```bash
bash train_graph_transformer.sh
```

This writes checkpoints to `./logXMTest_gt/` and trains the `--addGCN 2 --pheb_mode full` variant.
