...
def macro_f1(y_true, y_pred):
    ...
    true = y_true
    p_macro, r_macro, f_macro, support_macro = precision_recall_fscore_support(true, preds, average='macro')
...
def post_dataloader(batch):
    ...
    tokens, tokenst, input_ids, input_mask, target_ids, target_mask, sentiment_label, img_id, img_feat, adj = batch
...
def main():
    ...
    # Required parameters
    parser.add_argument("--dataset", default='twitter2015', type=str, choices=['twitter2015', 'twitter2017', 'animal', 'buildings', 'food', 'goods', 'human', 'plant', 'scencry'], help="The name of the task to train.")
    parser.add_argument("--data_dir", default='./data/Sentiment_Analysis/', type=str, help="The input data dir.")
    parser.add_argument("--imagefeat_dir", default='./data/Twitter_image/twitter2017_images', type=str, help="The directory of image features.")
    parser.add_argument("--output_dir", default="./logXMTest/twitter2015", type=str, help="The output directory where the model predictions and checkpoints will be written.")
    ...
    parser.add_argument('--roberta_model_dir', default='./model/roberta-base', type=str)
    parser.add_argument("--addgate", default=1, type=int, help="Whether to add gate, when 1 add.")
    parser.add_argument("--addGCN", default=1, type=int, help="Whether to add GCN, when 1 add.")
    parser.add_argument("--pheb_mode", default='full', type=str, choices=['full', 'zero', 'shared'], help="Graph Transformer edge-bias mode when addGCN=2.")
    parser.add_argument("--s_topk", default=0, type=int, help="Top-K sparse cross-modal connections per text node (0 disables).")
    parser.add_argument("--save", default=True, action='store_true', help="Whether to save model.")
    parser.add_argument("--train_batch_size", default=6, type=int, help="Total batch size for training.")
    parser.add_argument("--eval_batch_size", default=6, type=int, help="Total batch size for eval.")
    parser.add_argument("--SA_learning_rate", default=1e-5, type=float, help="The initial learning rate for Adam.")
    parser.add_argument("--num_train_epochs", default=10, type=int, help="Total number of training epochs to perform.")
    parser.add_argument("--warmup_proportion", default=0.1, type=float, help="Proportion of training to perform linear learning rate warmup for.")
    parser.add_argument('--seed', type=int, default=2020, help="random seed for initialization")
    parser.add_argument('--resnet_root', default='./resnet/resnet', help='path to pre-trained CNN models')
    ...
    data_txt_dir = args.data_dir.rsplit('/', 1)[0].rstrip('.pkl')
    for split in ['train', 'dev', 'test']:
        txt_file = os.path.join(data_txt_dir.replace('.pkl', ''), f"{split}.txt")
        if os.path.exists(txt_file):
            convert_to_pkl(txt_file)
            process(txt_file)

    train_dataset_SA = MyDataset(args.data_dir % str('train'), args.imagefeat_dir, tokenizer, max_seq_len=args.max_seq_length)
    train_dataloader_SA = Data.DataLoader(dataset=train_dataset_SA, shuffle=True, batch_size=args.train_batch_size, num_workers=0)

    eval_dataset_SA = MyDataset(args.data_dir % str('dev'), args.imagefeat_dir, tokenizer, max_seq_len=args.max_seq_length)
    eval_dataloader_SA = Data.DataLoader(dataset=eval_dataset_SA, shuffle=False, batch_size=args.eval_batch_size, num_workers=0)

    test_dataset = MyDataset(args.data_dir % str('test'), args.imagefeat_dir, tokenizer, max_seq_len=args.max_seq_length)
    ...
    # Use Faster R-CNN Feature Extractor
    from faster_rcnn import FasterRCNNFeatureExtractor
    encoder = FasterRCNNFeatureExtractor(device)
    # Faster R-CNN backbone (FPN) outputs 256 channels
    model = AHGT(args, img_feat_dim=256) 
    model.to(device)
    encoder.to(device)
    ...
    optimizer_SA = BertAdam(optimizer_grouped_parameters, lr=args.SA_learning_rate, warmup=args.warmup_proportion, t_total=num_train_steps)
    ...
    for train_idx in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for step, batch_SA in enumerate(tqdm(train_dataloader_SA, desc="Iteration")):
            ...
            SA_tokens, SA_tokenst, SA_input_ids, SA_input_mask, SA_target_ids, SA_target_mask, SA_sentiment_label, SA_img_id, SA_img_feat, SA_adj = post_dataloader(batch_SA)
            with torch.no_grad():
                imgs_f, img_mean, img_att = encoder(SA_img_feat)
            senti_pred = model(img_id=SA_img_id, input_ids=SA_input_ids, input_mask=SA_input_mask, target_ids=SA_target_ids, target_mask=SA_target_mask, img_feat=img_att, adj=SA_adj)
        ...
        logger.info("  SA Num examples = %d", eval_dataset_SA.number)
        ...
        for batch_SA in tqdm(eval_dataloader_SA, desc="Evaluating_SA"):
            ...
            with torch.no_grad():
                imgs_f, img_mean, img_att = encoder(SA_img_feat)
                SA_senti_pred = model(img_id=SA_img_id, input_ids=SA_input_ids, input_mask=SA_input_mask, target_ids=SA_target_ids, target_mask=SA_target_mask, img_feat=img_att, adj=SA_adj)
        ...
        result = {'Dev_senti_acc': senti_acc, 'Dev_senti_precision': senti_precision, 'Dev_senti_recall': senti_recall, 'Dev_senti_F_score': senti_F_score}
        ...
        for batch in tqdm(test_dataloader, desc="Testing"):
            tokens, tokenst, input_ids, input_mask, target_ids, target_mask, sentiment_label, img_id, img_feat, adj = post_dataloader(batch)
            with torch.no_grad():
                imgs_f, img_mean, img_att = encoder(img_feat)
                senti_pred = model(img_id=img_id, input_ids=input_ids, input_mask=input_mask, target_ids=target_ids, target_mask=target_mask, img_feat=img_att, adj=adj)
        ...
        result = {'Test_senti_acc': test_senti_acc, 'Test_senti_F_score': test_senti_F_score, 'Test_senti_precision': test_senti_precision, 'Test_senti_recall': test_senti_recall}
